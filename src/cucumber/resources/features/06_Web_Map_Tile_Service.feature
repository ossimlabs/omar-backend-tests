...
Feature: MapTileService
  ...
  Scenario: [WMTS-02] WMTS call to get the entire bounding box of an image
    When a call is made to WMTS for a jpeg image the for the entire bounding box of a WorldView2 MSI GeoTIFF image
  ...
  Scenario: [WMTS-03] WMTS call to get the outside entire bounding box of an image
    When a call is made to WMTS for a jpeg outside entire bounding box of a WorldView2 MSI GeoTIFF image
  ...
  Scenario: [WMTS-05] WMTS call for a subset of an image
    When a call is made to WMTS for jpeg of a subset of a WorldView2 MSI GeoTIFF image
    Then WMTS returns tiles that matches the validation jpeg image